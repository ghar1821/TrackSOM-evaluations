...
make.custom.timeseries.heatmap <- function(dat, timepoints, timepoint.col, 
                                           cluster.col, markers, rule,
                                           ...
                                           load.temp.data = FALSE, arrow.length = 2, 
                                           colours = 'spectral', custom.colours = FALSE,
                                           line.width = 1) {
  ...
  dat_chart[[population.col]] <- factor(dat_chart[[population.col]], levels=sort(unique(dat_chart[[population.col]])))
  ...
  xx <-  ggplot(dat_chart, aes_string(x = timepoint.col, y = to_plot_node)) + 
    geom_point(aes_string(size = "proportion", fill = population.col), pch=21) + 
  ...
  ggsave(filename = filename,
         ...
         height = plot.height,
         limitsize = FALSE)
  message(paste("Timeseries heatmap exported to", filename))
}
