...
setwd("/project/chronoclust/tracksom_eval/tracksom_with_merge/wnv_dataset")
...
if (option == 1) {
    ...
    tracksom.result <- TrackSOM(
        inputFiles = data.files.fullpath,
...
cell.dat <-
    ConcatenateClusteringDetails(
        ...
        timepoints = sapply(c(0:7), function(d)
            paste0("WNV_D", d))
    )
setwd(outdir)
